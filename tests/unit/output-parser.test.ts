...
import { DeployCloudRunInputs } from '../../src/deploy-cloudrun';
import { parseUpdateTrafficResponse, parseDeployResponse } from '../../src/output-parser';

const DEFAULT_INPUTS: DeployCloudRunInputs = {
  image: '',
  name: '',
  metadata: '',
  credentials: '',
...
describe('#output-parser', function () {
  describe('#parseUpdateTrafficResponse', function () {
    it('correctly parses update traffic outputs', function () {
      const output = `
      [
        {
          "displayPercent": "100%",
    ...
    `;
      const { url } = parseUpdateTrafficResponse(DEFAULT_INPUTS, output);
      expect(url).to.eq('https://test-basic-yaml-4goqgbaxqq-uc.a.run.app');
    });

    it('correctly parses update traffic with single tag', function () {
      const output = `
      [
        {
          "displayPercent": "0%",
    ...
    `;
      const { url } = parseUpdateTrafficResponse(DEFAULT_INPUTS, output);
      expect(url).to.eq('https://my-tag-1---test-basic-yaml-4goqgbaxqq-uc.a.run.app');
    });

    it('correctly parses update traffic with multiple tags', function () {
      const output = `
      [
        {
          "displayPercent": "20%",
    ...
    it('handles empty stdout', function () {
      expect(() => {
        parseUpdateTrafficResponse(DEFAULT_INPUTS, `[]`);
      }).to.throw('command response is empty: []');
    });
  });

  describe('#parseDeployResponse', function () {
    it('correctly parses deploy outputs', function () {
      const output = `
      {
    ...
    `;
      const { url } = parseDeployResponse(DEFAULT_INPUTS, output);
      expect(url).to.eq('https://action-test-cy7cdwrvha-uc.a.run.app');
    });

    it('correctly parses deploy outputs with tag', function () {
      const inputs = Object.assign({}, DEFAULT_INPUTS);
      inputs.tag = 'test';
    ...
    `;
      const { url } = parseDeployResponse(inputs, output);
      expect(url).to.eq('https://test---hello-4goqgbaxqq-uc.a.run.app');
    });

    it('handles invalid stdout', function () {
      expect(() => {
        parseDeployResponse(DEFAULT_INPUTS, `{}`);
      }).to.throw('invalid command response: {}');
    });
  });
});
